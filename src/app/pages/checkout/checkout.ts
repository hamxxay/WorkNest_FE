    ...
    if (navState?.fromBooking && navState?.bookingId) {
      ...
      if (b.challanNumber) {
        this.challan.set({
          ...
          securityDeposit: 0,
          totalAmount:     b.totalAmount ?? 0,
          subtotalAmount:  b.subtotalAmount ?? b.totalAmount ?? 0,
      ...
      this.bookingService.getChallan(bookingId).subscribe({
        next: (challanRes: any) => {
          ...
          this.challan.set({
            ...
            bookingId,
            spaceName,
            spaceNumber:        cd.spaceNumber ?? cd.spaceCode ?? '',
            ...
            monthlyRent:        cd.monthlyRent ?? cd.roomPrice ?? p?.rentAmount ?? 0,
            billingPeriod:      cd.billingPeriod ?? cd.billingPeriodLabel ?? 'Monthly',
            billingPeriodMonths: cd.billingPeriodMonths ?? 1,
            currentCycleAmount: cd.currentCycleAmount ?? totalPayable,
            totalContractAmount: cd.totalContractAmount ?? cd.contract?.totalContractAmount ?? 0,
            nextBillDueDate:    cd.nextBillDueDate ?? cd.nextBillingDate ?? cd.validUntil,
            nextBillingDate:    cd.nextBillingDate ?? cd.nextBillDueDate,
            balanceLeft:        cd.balanceLeft ?? cd.contract?.balanceLeft ?? 0,
            totalPaidAmount:    cd.totalPaidAmount ?? 0,
            rentAmount:         cd.monthlyRent ?? cd.roomPrice ?? p?.rentAmount ?? 0,
